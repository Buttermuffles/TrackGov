...
import { Clock, CheckCircle, AlertTriangle, FileText, Eye, ArrowRight } from 'lucide-react'
...
export default function Pending() {
  ...
  const myPending = useMemo(() => {
    ...
    const needsAck = inMyOffice.filter(d => { const lr = d.routingHistory[d.routingHistory.length - 1]; return lr && lr.toOfficeId === user.officeId && !lr.isAcknowledged })
    const assigned = inMyOffice.filter(d => d.currentAssigneeId === user.id)
    const overdue = inMyOffice.filter(d => d.dueDate && differenceInDays(new Date(d.dueDate), new Date()) < 0)
